<div class="content">
	<div>
		<div>
			<h1>Hotdogs</h1>
			<div> <a href="/menu/hotdog"><img src="/images/hotdog.jpg" width="434" height="327" alt=""></a> </div>
			<ul>
				<li>
					<h2><a href="/menu/hotdog">Hotdog Special</a></h2>
					<p>
                                            This a cheesy joke on hotdig.
					</p>
					<span class="price">99&cent;</span>
				</li>
				<li>
					<h2><a href="/menu/hotdog">Bacon Cheesedog</a></h2>
					<p>
						This website template has been designed by Free Website Templates for you, for free. You can replace all this text with your own text.
					</p>
					<span class="price">99&cent;</span>
				</li>
				<li>
					<h2><a href="/menu/hotdog">King Size Hotdog</a></h2>
					<p>
						This website template has been designed by Free Website Templates for you, for free. You can replace all this text with your own text.
					</p>
					<span class="price">$1</span>
				</li>
				<li class="last	">
					<h2><a href="/menu/hotdog">Kiddies Hotdog</a></h2>
					<p>
						A small burger with juicy beef patty, tomato, lettuce, bacon and cheese. Served with small fries and drink of your choice.
					</p>
					<span class="price">75&cent;</span>
				</li>
			</ul>
		</div>
	</div>
</div>
<div class="sidebar">
	<h1>Menu</h1>
	<ul class="navigation">
		<li class="first">
			<a href="/menu">BURGERS</a>
		</li>
		<li>
			<a class="active" href="/menu/hotdog">HOTDOGS</a>
		</li>
		<li>
			<a href="/menu/shake">SHAKES</a>
		</li>
		<li>
			<a href="/menu/breakfast">BREAKFAST</a>
		</li>
	</ul>
	<a href="/menu/hotdog" class="download">&nbsp;</a>
</div>
